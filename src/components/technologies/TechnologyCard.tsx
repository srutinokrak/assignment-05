...
const TechnologyCard = ({technology,setSelected}:ITech) => {
   ...
   return (
        <div>
          ...
          <div className="flex justify-between items-center">
           ...
           <button  className={`border rounded-3xl py-1 px-4 text-sm ${technology.badgeColor} ${technology.badge?"":"hidden"}`} >{technology.badge}</button>
          </div>
          ...
          <div className="text-gray-500  flex justify-between items-center  mt-3 text-xs">
             <button className="bg-gray-200 font-semibold border-gray-400 rounded-md px-2 py-1">{technology.category}</button>
         ...
         <div className="flex items-center gap-1 ">
           ...
           <button onClick={()=>handleSelected(technology) } className="mt-4 w-full bg-black text-white
            font-medium py-2 px-4 rounded-md ">Add to Stack</button>
        </div>
